...
class TestQuaternion : public Test {
	...
	public :
		...
		void testConstructors() {
			...
			const float PI_OVER_2 = PI * float(0.5);
			const float PI_OVER_4 = PI_OVER_2 * float(0.5);
		...
		void testOthersMethods() {
			...
			Quaternion test3 = Quaternion::slerp(mIdentity, quat, float(0.5));
		...
		void testOperators() {
			...
			test(vector1 == point);
			test(approxEqual(vector2.x, testVector2.x, float(10e-5)));
			test(approxEqual(vector2.y, testVector2.y, float(10e-5)));
			test(approxEqual(vector2.z, testVector2.z, float(10e-5)));
